...
pander(
data.frame(Top_Womens=head(lopsided.titles[order(-lopsided.balance)], n=5),
           Top_Mens=head(lopsided.titles[order(lopsided.balance)], n=5)),
justify=c("left", "left")
)
```

(Note that the job description for "VOCATIONAL INSTRUCTOR III - F/C" is 
"this is a maintenance heating, air conditioning, and refrigeration service and 
repair position supervising an inmate work crew")
...
When you look at the job classifications over all, sorted by the percent of women in the role,
we can see a subtle lowering of the salaries as we move from left (all men)
to right (all women):
```{r balancevsalary, echo=FALSE, message=FALSE}
lp <- lopsided.percent*100
plot(lp,lopsided.average, xlab="% Women in Job", ylab="Average Salary", col=rgb(0,0,0,0.25))
abline(lm(lopsided.average ~ lp))
```
...
The line through the data is the linear regression,
The one outlier in the upper right hand corner is Pamela L. Steward, Commissioner of Education,
