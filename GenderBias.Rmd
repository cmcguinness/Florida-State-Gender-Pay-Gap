...
* Women are being put into completely different job classifications even though
they are doing the same job as men. This would be an example of blatant bias, 
and I suspect it's not likely to be true; it would be too obvious to the workers
affected by it.
* Women are over-represented in lower paid professions.  You can certainly
argue that these professions are paid less because of societal bias, and I
won't disagree with you. But the fact that the state has a lower gap, overall, 
than is generally given for the country as a whole seems to suggest the state 
is fairer than average.
* There is some other difference between men and women's employment (seniority, 
geography, etc.) that explains the difference.  This sort of argument is
typically offered by those who feel the gender pay gap is the result of
perfectly innocent processes.

The one thing that seems clear is that the payroll data itself does not offer
us a smoking gun in a gender pay gap, and, in fact, suggests the State of
